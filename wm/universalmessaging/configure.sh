...
#sed -i "s|\(wrapper.java.library.path.1\)=\(.*\)|\1=$SAG_HOME/common/lib/tw-3.5.37|" $SERVER_COMMON_CONF_FILE
#sed -i "s|\(wrapper.java.classpath.12\)=\(.*\)|\1=$SAG_HOME/common/lib/tw-3.5.37/wrapper.jar|" $SERVER_COMMON_CONF_FILE
...
#sed -i "s|\(WRAPPER_CMD\)=\(.*\)|\1="\""$SAG_HOME/common/bin/wrapper-3.5.37"\""|" nserverdaemon
